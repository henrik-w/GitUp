...
- (instancetype)initWithDatabase:(NSString*)databasePath config:(NSString*)configPath localRepositoryContents:(NSString*)localPath error:(NSError**)error {
  ...
  if (configPath) {
    CALL_LIBGIT2_FUNCTION_GOTO(cleanup, git_config_add_file_ondisk, config, configPath.fileSystemRepresentation, GIT_CONFIG_LEVEL_LOCAL, NULL, false);
  } else {
    _configPath = [NSTemporaryDirectory() stringByAppendingPathComponent:[[NSProcessInfo processInfo] globallyUniqueString]];
    CALL_LIBGIT2_FUNCTION_GOTO(cleanup, git_config_add_file_ondisk, config, _configPath.fileSystemRepresentation, GIT_CONFIG_LEVEL_LOCAL, NULL, false);
  }
